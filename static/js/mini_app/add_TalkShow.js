...
export default {
  ...
  methods: {
    ...
    getDetail() {
      ...
      that.axios.post("/talk_show/edit", {
        id: that.talk_id
      }).then(data => {
        if (data) {
    ...
    save() {
      ...
      that.$refs.form_data.validate(valid => {
        ...
        var url = '/talk_show/create';
        if (that.talk_id) {
          formData.id = that.talk_id;
          url = '/talk_show/update';
        }

        that.axios.post(url, formData).then(() => {
          that.$message.success(that.talk_id ? '修改成功' : '添加成功');
          that.$router.go(-1);
        });
      });
    },
